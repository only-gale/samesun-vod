...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
	 ...
	 * @return
	 */
	public TSType getType(String typecode, String typegroupcode) {
		TSType actType = null;
		if(StringUtil.isNotEmpty(typecode) && StringUtil.isNotEmpty(typegroupcode)){
			TSTypegroup actTygroup = getTypeGroupByCode(typegroupcode);
			List<TSType> types = actTygroup.getTSTypes();
			for(TSType t : types){
	 ...
	 * @return
	 */
	public List<TSType> getTypes(TSTypegroup tsTypegroup) {
		return commonDao.findByProperty(TSType.class, "TSTypegroup", tsTypegroup);
	}
